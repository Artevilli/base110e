...
char *ExpandPathAndArchive (const char *path)
{
	...
	if (archive)
	{
		len = snprintf (archivename, sizeof(archivename), "%s/%s", archivedir, path);

                if (len >= sizeof(archivename))
                {
                  Error("expandpathandarchive path too long");
                }
                else
                {
