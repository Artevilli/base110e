...
*/
void
CG_GhostBuildable(buildable_t buildable)
{
  ...
  CG_PositionAndOrientateBuildable(ps->viewangles, entity_origin, tr.plane.normal, ps->clientNum, mins, maxs, ent[0].axis, ent[0].origin);

  //offset on the Z axis if required
  VectorMA(ent[0].origin, BG_FindZOffsetForBuildable(buildable), tr.plane.normal, ent[0].origin);
...
static void CG_BuildableHealthBar( centity_t *cent )
{
  ...
  progress = (float)health / B_HEALTH_MASK;
