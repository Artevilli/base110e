...
*/
static void
TimeShiftLerp(float frac, const vec3_t start, const vec3_t end, vec3_t result)
{
...
G_TimeShiftClient(gentity_t *ent, int time, qbool debug, gentity_t *debugger)
{
  ...
  if (j != k)
  {
    ...
    {
      float frac = (float)(time - client->history[j].leveltime) / (float)(client->history[k].leveltime - client->history[j].leveltime);
...
*/
void
G_TimeShiftAllClients(int ltime, gentity_t *skip)
{
  int i;
  gentity_t *ent;
...
*/
void
G_DoTimeShiftFor(gentity_t *ent)
{	
  int time;
  ...
  if (g_unlagged.integer && ent->client->pers.useUnlagged)
  {
...
*/
void
G_UnTimeShiftClient(gentity_t *ent)
{
  //if it was saved
...
*/
void
G_UnTimeShiftAllClients(gentity_t *skip) 
{
  int i;
  gentity_t *ent;
  qbool linked;
...
*/
void
G_UndoTimeShiftFor(gentity_t *ent)
{
...
static void
G_PredictPlayerClipVelocity(const vec3_t in, const vec3_t normal, vec3_t out)
{
  float backoff;
...
*/
void
G_PredictPlayerStepSlideMove(gentity_t *ent, float frametime)
{
...
*/
void
G_PredictPlayerMove(gentity_t *ent, float frametime)
{
